...
namespace ServiceAPI
{
    public partial class Form1 : Form
    {
        ...
        private void GetAllServices()
        {
            ...
            foreach (ServiceController scTemp in scServices)
            {
                ...
                try
                {
                    ...
                    string[] row = new string[] { scTemp.ServiceName, ServiceId, wmiService["Description"].ToString(), Status, wmiService["StartName"].ToString() };
                    row_check = row;
                    listView1.Items.Add(new ListViewItem(row));
                }
                catch(Exception ex)
                {
                    Console.WriteLine(row_check);
                }

            }
        }

        private void RefreshListServ()
        {
        ...
        public static class LsaLib
        {
            ...
            [DllImport("Secur32.dll", SetLastError = false)]
            private static extern NtStatus LsaGetLogonSessionData(IntPtr luid, out IntPtr ppLogonSessionData);

            [DllImport("secur32.dll", SetLastError = false)]
            private static extern NtStatus LsaFreeReturnBuffer([In] IntPtr buffer);

            private static string LSA2string(LSA_UNICODE_STRING data)
            {
                return Marshal.PtrToStringUni(data.Buffer).Trim();
            }

            public static List<string> GetLogonSessionData()
            {
                ...
                unsafe
                {
                    ...
                    for (ulong i = 0; i < count; i++)   //for each pointer in the array
                    {
                        ...
                        NtStatus state = LsaGetLogonSessionData(iter, out sessionData);
                        if (state == NtStatus.Success)
                        {
                            ...
                            if (data.PSiD != IntPtr.Zero)
                            {
                                ...
                                string username = LSA2string(data.Username);      //get the account username
                                string domain = LSA2string(data.LoginDomain); //domain for this account  
                                string authpackage = LSA2string(data.AuthenticationPackage);    //authentication package
                                ...
                                // display
                                sList.Add($"{username} ({domain}) " + Environment.NewLine +
                                    $"    Session:     {data.Session}" + Environment.NewLine +
                                    $"    Authpackage: {authpackage}" + Environment.NewLine +
                                    $"    type:        {secType}" + Environment.NewLine +
                                    $"    Time:        {time.ToLocalTime()}" + Environment.NewLine +
                                    Environment.NewLine
                                 );
                            }
        ...
        {
            List<string> sessions = LsaLib.GetLogonSessionData();
